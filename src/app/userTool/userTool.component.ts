...
export class UserToolComponent implements OnInit {
  ...
  constructor( config: NgbModalConfig, private mail:EmailService,private images:ImagesService,private filter:FilterDetailsService,private modalService: NgbModal,private titleService: Title, private metaService: Meta) {
...
console.log('initCropper');
  ...
  //   console.log(result);
  // }
  clickFilter(filter:FiltersArray):void{
    let objectUrl: any;
    let img2 :any= document.getElementById('img2');
    let img4 :any= document.getElementById('img4');
    // const result:any = (e.target as HTMLInputElement).src;
    this.images.downloadFile(filter.imageFilterUrl)
    .subscribe((response) => {
  ...
  canvas.toBlob((imgBlob: any) => {
    imgFile = new File([imgBlob], 'filteredImg.png', { type: 'image/png' });
    this.mail.sendmail(email, imgFile).subscribe(res => this.closePopup());
  },'image/png');

  // this.formData.ImageFilterUrl=this.file;
  // this.formData.name=this.filename;
